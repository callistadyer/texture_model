...
if args.model == 'UNet':
    ...
    unet_args = SimpleNamespace(**args_dict)  # architecture config: num layers, kernel sizes, etc.
    unet_base = UNet_flex(unet_args)          # build the empty network in the right shape (random weights)
    state = torch.load(weights_path, map_location='cpu', weights_only=False)  # load learned weights from disk
    unet_base.load_state_dict(state)          # fill each layer with its corresponding learned weights
    unet_base.eval()                          # switch to inference mode (disables batchnorm updates etc.)
